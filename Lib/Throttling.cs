...
namespace stratfaucet.Lib
{
    public class Throttling
    {
        ...
        public static Recipient WaitForTransaction(string address)
        {
            ...
            while (waitCount < timeout)
            {
                ...
                if (Transactions.TryGetValue(address, out recipient))
                {
                    ...
                    if (recipient.is_error && !recipient.is_sent)
                    {
                        Console.WriteLine("Removing " + recipient.address);
                        Transactions.Remove(recipient.address, out Recipient rec);
                        if(recipient.errorText.Length > 0){
                          throw new FaucetException(recipient.errorText);
                        }else {
        ...
        public static void Manage()
        {
            ...
            {
                if ((transaction.is_error && transaction.errorCount > 5) || transaction.is_sent)
                {
                    Console.WriteLine("Removing " + transaction.address);
